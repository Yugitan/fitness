-- ============================================================
-- 健身记录Web应用 - 测试数据脚本
-- 请在执行schema.sql建表后执行此脚本
-- ============================================================

USE fitness;

-- ============================================================
-- 一、默认管理员账号
-- 用户名: admin  密码: 123456 (BCrypt加密)
-- ============================================================
INSERT INTO user (username, password, nickname, role, status) VALUES
('admin', '$2a$10$.vQCZ3G.e7t0hqr/gL/gVuNgtnkPaJB8TyPKc8MV5/FAoWcA10cmO', '系统管理员', 1, 1),
('test',  '$2a$10$.vQCZ3G.e7t0hqr/gL/gVuNgtnkPaJB8TyPKc8MV5/FAoWcA10cmO', '测试用户', 0, 1);

-- 测试用户训练记录（用户ID=2 test）
INSERT INTO training_record (user_id, train_date, duration, total_sets, total_reps, difficulty, is_completed, notes, create_time, update_time) VALUES
(2, '2026-05-16', 60, 15, 160, 1, 1, '今天第一次训练，感觉很棒', NOW(), NOW());
SET @test_record_id = LAST_INSERT_ID();
INSERT INTO training_record_detail (record_id, exercise_id, set_number, reps, weight, is_completed, sort_order, create_time, update_time) VALUES
(@test_record_id, 1, 1, 12, 40.0, 1, 1, NOW(), NOW()),
(@test_record_id, 1, 2, 10, 45.0, 1, 2, NOW(), NOW()),
(@test_record_id, 1, 3,  8, 50.0, 1, 3, NOW(), NOW()),
(@test_record_id, 2, 1, 12, 15.0, 1, 4, NOW(), NOW()),
(@test_record_id, 2, 2, 10, 17.5, 1, 5, NOW(), NOW()),
(@test_record_id, 5, 1, 15,  0.0, 1, 6, NOW(), NOW()),
(@test_record_id, 5, 2, 12,  0.0, 1, 7, NOW(), NOW());

-- 测试用户训练计划
INSERT INTO plan (user_id, group_id, title, description, target_body_part, difficulty_level, train_days, is_public, create_time, update_time) VALUES
(2, 1, '测试用户入门计划', '刚开始健身，从基础动作开始', '全身', 1, 3, 1, NOW(), NOW());
SET @test_plan_id = LAST_INSERT_ID();
INSERT INTO plan_detail (plan_id, exercise_id, day_number, sets, reps, weight, rest_seconds, sort_order, create_time, update_time) VALUES
(@test_plan_id, 5,  1, 3, 12,  0.0, 60, 1, NOW(), NOW()),
(@test_plan_id, 18, 1, 3, 30,  0.0, 90, 2, NOW(), NOW()),
(@test_plan_id, 12, 2, 3, 12,  5.0, 60, 3, NOW(), NOW()),
(@test_plan_id, 17, 2, 3, 15, 10.0, 60, 4, NOW(), NOW()),
(@test_plan_id, 20, 3, 3, 10, 30.0, 90, 5, NOW(), NOW());

-- ============================================================
-- 二、健身动作测试数据 (至少20个)
-- ============================================================

-- 【胸部】5个动作
INSERT INTO exercise (name, category, description, key_points, precautions, sort_weight) VALUES
('平板杠铃卧推', '胸部',
 '平板杠铃卧推是最经典的胸部训练动作，主要锻炼胸大肌中部，同时也能锻炼到三角肌前束和肱三头肌。躺在平板凳上，双脚平放地面，双手握住杠铃，宽度略宽于肩。将杠铃缓慢下放至胸部下方，然后用力推起至手臂伸直。',
 '1. 核心收紧，保持身体稳定\n2. 下放时肘部与身体呈45度角\n3. 推起时不要锁死肘关节\n4. 全程控制杠铃速度\n5. 呼吸节奏：下放吸气，推起呼气',
 '1. 初学者应在有人保护的情况下进行\n2. 不要过度追求大重量\n3. 下放时杠铃触碰胸部即可，不要弹起\n4. 肩部有伤病者需谨慎\n5. 热身组必不可少',
 100),

('上斜哑铃卧推', '胸部',
 '上斜哑铃卧推重点刺激胸大肌上部和锁骨部位，能让胸部看起来更加饱满。将长凳调整至30-45度倾斜角度，双手各握一只哑铃，从胸部两侧向上推起，在最高点不要完全锁死肘关节。',
 '1. 长凳角度控制在30-45度\n2. 哑铃下放至胸部两侧位置\n3. 推起时感受上胸发力\n4. 最高点停顿1秒挤压胸肌\n5. 腕关节保持中立位',
 '1. 角度过大(超过45度)会过多用到肩部\n2. 选择合适重量，避免肩关节受伤\n3. 下放时控制速度\n4. 大重量时请找保护者',
 90),

('哑铃飞鸟', '胸部',
 '哑铃飞鸟是胸部塑形的经典孤立动作，能有效拉伸和收缩胸大肌，增加胸部宽度。仰卧在平凳上，双手持哑铃在胸部上方，掌心相对，缓慢向两侧打开手臂至胸部有充分拉伸感，然后沿弧线回到起始位置。',
 '1. 手臂保持微屈，不要完全伸直\n2. 打开时感受胸肌拉伸\n3. 收回时想象"拥抱大树"\n4. 使用中等重量，注重肌肉感受\n5. 全程控制动作速度',
 '1. 不建议使用过重重量\n2. 下放幅度因人而异，感到胸肌拉伸即可\n3. 肩部不舒服时应立即停止\n4. 可以变换长凳角度刺激不同部位',
 70),

('绳索夹胸', '胸部',
 '绳索夹胸使用龙门架完成，能持续给胸肌施加张力，是雕刻胸肌中缝和下部线条的绝佳动作。站立在龙门架中间，双手握住高位滑轮绳索，身体微微前倾，双手向身体前方中央夹拢。',
 '1. 身体保持稳定，不要过度摆动\n2. 夹至胸前时停顿挤压胸肌\n3. 绳索轨迹呈弧线\n4. 回放时控制速度，保持张力\n5. 站姿可调节以刺激不同胸部区域',
 '1. 从轻重量开始，掌握动作后再加重\n2. 注意肘部角度不要过大\n3. 身体不要过度摇晃借力\n4. 保持核心收紧保护下背',
 65),

('俯卧撑', '胸部',
 '俯卧撑是最基础的自重训练动作，无需器械，随时随地可练。主要锻炼胸大肌、三角肌前束和肱三头肌，同时核心肌群也参与稳定。双手与肩同宽或略宽于肩，身体保持一条直线，弯曲肘部将身体下放，然后推起。',
 '1. 身体从头到脚保持一条直线\n2. 核心收紧，臀部不要塌陷或抬起\n3. 下放至胸部接近地面\n4. 肘部与身体呈45度角\n5. 全程控制，不要利用惯性',
 '1. 手腕不适时可使用俯卧撑把手\n2. 初学者可从跪姿俯卧撑开始\n3. 变式：宽距(练胸)/窄距(练手臂)\n4. 呼吸：下放吸气，推起呼气',
 50),

-- 【背部】5个动作
('引体向上', '背部',
 '引体向上是背部训练的黄金动作，主要锻炼背阔肌，同时也能训练到肱二头肌和核心肌群。双手略宽于肩握住单杠，身体悬垂，用力将身体上拉至下巴超过单杠，然后缓慢下放。',
 '1. 握距略宽于肩\n2. 上拉时感受背部发力\n3. 最高点停顿1秒\n4. 下放时完全伸直手臂\n5. 避免身体摆动借力',
 '1. 初学者可使用弹力带辅助\n2. 肩关节有伤者慎练\n3. 不要借力摆动身体\n4. 正手握杠主要练背，反手练手臂更多',
 95),

('杠铃划船', '背部',
 '杠铃划船是增加背部厚度的最佳动作之一，主要针对背阔肌、菱形肌和斜方肌中下部。双脚与肩同宽站立，屈髋俯身至躯干与地面约45度，双手正握杠铃，沿大腿方向将杠铃拉向腹部。',
 '1. 保持下背平直，不要弓背\n2. 杠铃沿大腿方向拉动\n3. 顶峰收缩感受背部发力\n4. 下放时完全伸展背部\n5. 使用腿部力量辅助稳定',
 '1. 下背有伤者建议使用器械划船替代\n2. 不要用过重重量导致弯腰\n3. 保持核心收紧保护腰椎\n4. 建议佩戴举重腰带做大重量训练',
 85),

('高位下拉', '背部',
 '高位下拉是引体向上的绝佳替代动作，适合不同训练水平的人群。坐姿面对高位下拉器械，双手略宽于肩握住横杆，将横杆拉至上胸位置，感受背部收缩，然后缓慢放回。',
 '1. 背部挺直，核心收紧\n2. 下拉至锁骨或上胸位置\n3. 肩胛骨先启动，再用手臂拉动\n4. 回放时完全伸直手臂感受拉伸\n5. 身体可略微后倾(不超过30度)',
 '1. 不要在颈后下拉(增加肩部受伤风险)\n2. 重量不要过大，保证动作质量\n3. 全程控制，不要利用惯性\n4. 选择适合的握把附件刺激不同区域',
 80),

('坐姿划船', '背部',
 '坐姿划船能有效刺激背阔肌中部和菱形肌，是改善体态、增强背部厚度的优秀动作。坐在划船机上，双脚踩踏板，双手握住V型把手，背部挺直，将把手拉向腹部，感受背部收缩。',
 '1. 背部始终保持挺直\n2. 拉动时肩胛骨向后夹紧\n3. 把手拉至腹部位置\n4. 回放时肩部前伸感受拉伸\n5. 控制速度，不要用腰部借力',
 '1. 选择合适重量，先掌握动作\n2. 下背保持稳定\n3. 可以变换握把刺激不同背部区域\n4. 适合与高位下拉组合超级组训练',
 70),

('单臂哑铃划船', '背部',
 '单臂哑铃划船是经典的背部单侧训练动作，能有效纠正左右背部不对称的问题。一只手和同侧膝盖支撑在长凳上，另一只手持哑铃，背部保持平直，将哑铃拉至髋部位置，顶峰收缩后缓慢下放。',
 '1. 支撑手和膝盖在长凳上保持稳定\n2. 背部与地面平行\n3. 哑铃拉至髋部或下肋位置\n4. 顶峰收缩1-2秒\n5. 下放时感受背部拉伸',
 '1. 不要旋转躯干借力\n2. 使用可控的重量\n3. 保持颈部与脊柱一条直线\n4. 两侧各做完一组后再交换',
 75),

-- 【肩部】4个动作
('哑铃推举', '肩部',
 '哑铃推举是发展肩部力量和体积的核心动作，主要锻炼三角肌前束和中束，同时肱三头肌也参与发力。坐在直角凳上，双手各持哑铃举至肩部高度，向上推起至手臂接近伸直，然后缓慢下放。',
 '1. 背部紧贴靠背\n2. 哑铃直上直下推起\n3. 最高点不要锁死肘关节\n4. 下放至耳侧位置即可\n5. 核心收紧稳定身体',
 '1. 肩部有伤者从极轻重量开始\n2. 不要弓背借力\n3. 可以使用杠铃替代做杠铃推举\n4. 站姿推举需要更稳定的核心力量',
 85),

('哑铃侧平举', '肩部',
 '哑铃侧平举是打造宽阔肩部的最佳孤立动作，主要锻炼三角肌中束。站姿或坐姿，双手持哑铃放在身体两侧，肘部微屈，将哑铃向两侧平举至与肩同高，然后缓慢下放。',
 '1. 肘部保持微屈(约10-15度)\n2. 哑铃举至肩部高度即可，不要过高\n3. 下放时控制速度(2-3秒)\n4. 小重量、多次数效果更佳\n5. 上身不要摇晃借力',
 '1. 不要用过重重量(这是最常见错误)\n2. 最高点小拇指略高于大拇指(倒水姿势)\n3. 肩部弹响或疼痛应立即停止\n4. 建议用2-5kg哑铃起步',
 70),

('哑铃前平举', '肩部',
 '哑铃前平举主要锻炼三角肌前束，能让肩膀从正面看起来更加饱满立体。站姿，双手持哑铃放在大腿前方，手臂伸直或微屈，将哑铃向前方举起至肩部高度，然后缓慢下放。',
 '1. 双手交替或同时进行均可\n2. 哑铃举至肩高即可\n3. 不要借力摆动身体\n4. 肘部保持微屈\n5. 控制下放速度',
 '1. 前束在卧推中已训练较多，注意训练量\n2. 使用轻到中等重量\n3. 可以旋转手臂(锤式)增加变化\n4. 保持背部挺直',
 60),

('俯身飞鸟', '肩部',
 '俯身飞鸟主要锻炼三角肌后束和上背部肌群，是改善圆肩驼背体态的重要动作。俯身至躯干几乎与地面平行，双手持哑铃自然下垂，肘部微屈，将哑铃向两侧打开至肩部高度，感受后束收缩。',
 '1. 躯干与地面几乎平行\n2. 肘部保持微屈角度不变\n3. 最高点挤压后束\n4. 下放时控制速度\n5. 使用较轻重量，注重感受',
 '1. 重心不稳时可靠在斜凳上完成\n2. 后束训练很容易用到背部借力，专注感受\n3. 不要甩动哑铃\n4. 建议放在训练后半程进行',
 55),

-- 【手臂】4个动作
('杠铃弯举', '手臂',
 '杠铃弯举是最经典的肱二头肌训练动作，能有效增加手臂围度。站姿，双手与肩同宽正握杠铃，上臂保持固定贴在身体两侧，屈肘将杠铃弯举至肩部高度，顶峰收缩后缓慢下放。',
 '1. 上臂固定，只有前臂移动\n2. 顶峰收缩1秒挤压二头肌\n3. 下放时完全伸直手臂\n4. 身体不要摆动借力\n5. 可以使用EZ曲杠减少手腕压力',
 '1. 不要用过重重量导致摆动\n2. 手腕保持中立位\n3. 全程控制，尤其是下放阶段\n4. 可以变换握距刺激不同二头肌区域',
 75),

('哑铃锤式弯举', '手臂',
 '锤式弯举在锻炼肱二头肌的同时能很好地刺激肱肌(位于二头肌下方)，使手臂看起来更厚实。站姿，双手持哑铃，掌心相对(锤式握法)，上臂固定，将哑铃弯举至肩部，顶峰收缩后下放。',
 '1. 掌心始终保持相对(锤式握法)\n2. 上臂固定不动\n3. 顶峰感受肱肌和二头肌收缩\n4. 双侧交替或同时进行\n5. 控制速度，注重肌肉感受',
 '1. 锤式握法对手腕更友好\n2. 适合与标准弯举组合训练\n3. 重量不宜过大\n4. 可以在牧师凳上完成减少借力',
 65),

('绳索下压', '手臂',
 '绳索下压是肱三头肌的经典塑形动作，能有效训练三头肌外侧头，打造手臂的"马蹄形"线条。面对龙门架，双手握住高位绳索，肘部固定在身体两侧，将绳索下压至手臂完全伸直，感受三头肌收缩。',
 '1. 肘部固定在身体两侧不动\n2. 下压至手臂完全伸直\n3. 顶峰停留1秒挤压三头肌\n4. 回放时控制速度\n5. 上身前倾不超过15度',
 '1. 不要用过重重量\n2. 可以选择不同的握把附件\n3. 手腕保持中立位\n4. 下压到底时手可以略微外旋增加收缩',
 65),

('窄距卧推', '手臂',
 '窄距卧推是增加肱三头肌力量和体积的高效复合动作。仰卧在平板凳上，双手窄握杠铃(双手间距约15-20cm)，将杠铃下放至下胸位置，然后用力推起至手臂伸直，全程感受三头肌发力。',
 '1. 双手间距15-20cm(与肩同宽或略窄)\n2. 杠铃下放至下胸位置\n3. 推起时专注于三头肌发力\n4. 肘部贴近身体\n5. 有人保护时可用较大重量',
 '1. 新手建议使用史密斯机练习\n2. 不要用过窄的握距(增加手腕压力)\n3. 肘部保持贴近身体\n4. 大重量训练务必有人保护',
 70),

-- 【腿部】4个动作
('杠铃深蹲', '腿部',
 '杠铃深蹲被称为"训练之王"，是最重要的下肢复合动作，主要锻炼股四头肌、臀大肌和核心肌群。将杠铃放置在上背部斜方肌上，双脚与肩同宽或略宽，下蹲至大腿与地面平行或更低，然后站起。',
 '1. 背部始终保持挺直\n2. 膝盖与脚尖方向一致\n3. 下蹲深度：大腿至少与地面平行\n4. 重心放在脚掌中部\n5. 起立时臀部和腿部同时发力',
 '1. 初学者先用空杠练习动作\n2. 膝盖不要内扣\n3. 核心全程收紧保护腰椎\n4. 建议使用深蹲架和安全杠\n5. 有膝盖伤病者需评估后再做',
 100),

('硬拉', '腿部',
 '硬拉是全身性的王牌训练动作，主要锻炼后链肌群：竖脊肌、臀大肌、腘绳肌，同时也能训练到背部、核心和握力。双脚与髋同宽站立，正反手握杠，背部挺直，通过伸髋发力将杠铃拉起至身体直立。',
 '1. 杠铃紧贴小腿和大腿拉起\n2. 背部始终挺直\n3. 核心收紧，全程保持张力\n4. 拉起时推送髋部向前\n5. 下放时先屈髋再屈膝',
 '1. 这是技术性最强的动作，建议请教练指导\n2. 一定要从轻重量开始学习\n3. 下背有伤者考虑用六角杠铃替代\n4. 绝对不要弓背！这是最大的安全风险',
 95),

('腿举', '腿部',
 '腿举(倒蹬)是安全的腿部大重量训练动作，能有效刺激股四头肌、臀大肌和腘绳肌。坐在腿举机上，双脚放在踏板上与肩同宽，将重量蹬起至腿接近伸直，然后缓慢屈膝下放至90度。',
 '1. 下背和臀部紧贴靠垫\n2. 下放至膝盖约90度即可\n3. 蹬起时不要完全锁死膝盖\n4. 脚位不同刺激不同区域\n5. 控制动作全程速度',
 '1. 膝盖不要锁死\n2. 脚位高偏练臀，脚位低偏练腿\n3. 不要放到底(减少腰椎压力)\n4. 从空机开始递增重量',
 80),

('哑铃弓步蹲', '腿部',
 '哑铃弓步蹲是优秀的单侧腿部训练动作，能改善左右腿力量不平衡，同时锻炼核心稳定性。双手持哑铃自然下垂，一脚向前跨出一大步，双膝弯曲下蹲至后膝接近地面，然后前腿发力回到起始位置。',
 '1. 前膝不要超过脚尖\n2. 上身保持挺直\n3. 下蹲至后膝接近地面\n4. 前腿脚跟发力站起\n5. 核心收紧保持平衡',
 '1. 平衡能力差者先徒手练习\n2. 膝盖有伤者减小步幅\n3. 跨步距离不宜太短或太长\n4. 可以先在原地做分腿蹲进阶',
 70),

-- 【核心】4个动作
('平板支撑', '核心',
 '平板支撑是训练核心稳定性的最佳自重动作，能同时锻炼腹直肌、腹横肌、腹斜肌和下背肌群。俯卧在垫子上，用肘部和脚尖支撑身体，身体从头到脚保持一条直线，核心收紧保持姿势。',
 '1. 身体从头到脚一条直线\n2. 腹部收紧，不要塌腰\n3. 臀部不要抬起\n4. 正常呼吸，不要憋气\n5. 从30秒开始，逐步增加时间',
 '1. 腰部疼痛应立即停止\n2. 可以从短时间多组开始\n3. 变式：侧平板支撑训练腹斜肌\n4. 核心训练放在训练末尾进行',
 45),

('仰卧起坐', '核心',
 '仰卧起坐是最经典的核心训练动作，主要锻炼腹直肌。仰卧在垫子上，屈膝约90度，脚底平放地面，双手放在头后或交叉在胸前，用腹部力量将上半身卷起，然后缓慢下放。',
 '1. 用腹部力量卷起身体，不要用颈部发力\n2. 手不要用力拉头部\n3. 下放时控制速度\n4. 下放时肩部不要完全贴地保持张力\n5. 呼吸：卷起时呼气，下放时吸气',
 '1. 双手不要抱头用力拉(伤颈椎)\n2. 下背贴紧地面\n3. 可以双脚固定但要确保核心发力\n4. 腹肌不需要每天大量训练',
 50),

('卷腹', '核心',
 '卷腹比仰卧起坐对腰椎更友好，是目前科学界更推荐的核心训练动作。仰卧，屈膝，双手放在胸前或大腿上，仅将肩胛骨抬离地面（上背部离地即可），感受腹肌收缩后缓慢下放。',
 '1. 仅上背部离地(约30度)\n2. 下背始终贴紧地面\n3. 顶峰停留1秒挤压腹肌\n4. 下放时吸气，卷起时呼气\n5. 动作幅度小而精，注重收缩',
 '1. L腰椎间盘问题者应选择此动作而非仰卧起坐\n2. 颈部放松，眼睛看天花板\n3. 可以增加负重(胸前抱哑铃片)\n4. 做15-20次/组为宜',
 55),

('俄罗斯转体', '核心',
 '俄罗斯转体是锻炼腹斜肌（侧腹）的经典动作，能塑造腰部线条，增强核心旋转力量。坐姿，身体后仰约45度，双腿抬起或脚跟点地，双手握在一起或持重物，向左右两侧交替旋转躯干。',
 '1. 身体保持V字形稳定\n2. 旋转来自胸椎而非腰椎\n3. 可用药球或哑铃增加难度\n4. 保持正常呼吸\n5. 视线跟随双手运动',
 '1. 腰椎间盘有问题者应避免此动作\n2. 初学者脚跟点地降低难度\n3. 控制旋转幅度\n4. 两侧各完成一次才算一次完整动作',
 50);

-- ============================================================
-- 三、默认训练计划分组
-- ============================================================
INSERT INTO plan_group (name, sort_weight) VALUES
('新手入门', 100),
('增肌计划', 90),
('减脂计划', 80),
('力量突破', 70);

-- ============================================================
-- 四、系统配置初始数据
-- ============================================================
INSERT INTO system_config (config_key, config_value, config_desc) VALUES
('site_name', 'Fitness健身记录', '网站名称'),
('site_logo', '/static/images/logo.png', '网站Logo路径'),
('site_icp', '© 2025 Fitness健身记录', '页脚备案信息'),
('site_description', '个人健身训练记录与管理平台', '网站描述');

-- ============================================================
-- 五、管理员训练记录（用户ID=1 admin，用于测试数据加载）
-- ============================================================

-- 训练记录1：胸部训练
INSERT INTO training_record (user_id, train_date, duration, total_sets, total_reps, difficulty, is_completed, notes, create_time, update_time) VALUES
(1, '2026-05-17', 75, 20, 240, 2, 1, '今天胸部状态不错，卧推又有突破', NOW(), NOW());
SET @record1_id = LAST_INSERT_ID();
INSERT INTO training_record_detail (record_id, exercise_id, set_number, reps, weight, is_completed, sort_order, create_time, update_time) VALUES
(@record1_id, 1,  1, 12,  60.0, 1, 1, NOW(), NOW()),
(@record1_id, 1,  2, 10,  70.0, 1, 2, NOW(), NOW()),
(@record1_id, 1,  3,  8,  80.0, 1, 3, NOW(), NOW()),
(@record1_id, 1,  4,  6,  85.0, 1, 4, NOW(), NOW()),
(@record1_id, 2,  1, 12,  25.0, 1, 5, NOW(), NOW()),
(@record1_id, 2,  2, 10,  27.5, 1, 6, NOW(), NOW()),
(@record1_id, 2,  3, 10,  27.5, 1, 7, NOW(), NOW()),
(@record1_id, 3,  1, 15,  12.5, 1, 8, NOW(), NOW()),
(@record1_id, 3,  2, 12,  15.0, 1, 9, NOW(), NOW()),
(@record1_id, 3,  3, 12,  15.0, 1, 10, NOW(), NOW());

-- 训练记录2：背部训练
INSERT INTO training_record (user_id, train_date, duration, total_sets, total_reps, difficulty, is_completed, notes, create_time, update_time) VALUES
(1, '2026-05-15', 70, 18, 180, 3, 1, '引体向上每组都能完成，进步明显', NOW(), NOW());
SET @record2_id = LAST_INSERT_ID();
INSERT INTO training_record_detail (record_id, exercise_id, set_number, reps, weight, is_completed, sort_order, create_time, update_time) VALUES
(@record2_id, 6,  1, 10,   0.0, 1, 1, NOW(), NOW()),
(@record2_id, 6,  2, 10,   0.0, 1, 2, NOW(), NOW()),
(@record2_id, 6,  3,  8,   0.0, 1, 3, NOW(), NOW()),
(@record2_id, 7,  1, 10,  60.0, 1, 4, NOW(), NOW()),
(@record2_id, 7,  2, 10,  65.0, 1, 5, NOW(), NOW()),
(@record2_id, 7,  3,  8,  70.0, 1, 6, NOW(), NOW()),
(@record2_id, 7,  4,  8,  70.0, 1, 7, NOW(), NOW()),
(@record2_id, 8,  1, 12,  55.0, 1, 8, NOW(), NOW()),
(@record2_id, 8,  2, 12,  55.0, 1, 9, NOW(), NOW()),
(@record2_id, 8,  3, 10,  60.0, 1, 10, NOW(), NOW());

-- 训练记录3：腿部训练
INSERT INTO training_record (user_id, train_date, duration, total_sets, total_reps, difficulty, is_completed, is_deleted, notes, create_time, update_time) VALUES
(1, '2026-05-13', 65, 16, 160, 2, 1, 0, '深蹲加了5kg，最后一组有点吃力', NOW(), NOW());
SET @record3_id = LAST_INSERT_ID();
INSERT INTO training_record_detail (record_id, exercise_id, set_number, reps, weight, is_completed, sort_order, create_time, update_time) VALUES
(@record3_id, 20,  1, 12,  80.0, 1, 1, NOW(), NOW()),
(@record3_id, 20,  2, 10,  90.0, 1, 2, NOW(), NOW()),
(@record3_id, 20,  3,  8, 100.0, 1, 3, NOW(), NOW()),
(@record3_id, 20,  4,  8, 100.0, 1, 4, NOW(), NOW()),
(@record3_id, 21,  1, 10,  80.0, 1, 5, NOW(), NOW()),
(@record3_id, 21,  2,  8,  90.0, 1, 6, NOW(), NOW()),
(@record3_id, 21,  3,  6, 100.0, 1, 7, NOW(), NOW()),
(@record3_id, 22,  1, 15, 150.0, 1, 8, NOW(), NOW()),
(@record3_id, 22,  2, 12, 170.0, 1, 9, NOW(), NOW()),
(@record3_id, 22,  3, 10, 190.0, 1, 10, NOW(), NOW());

-- 训练记录4：未完成的手臂训练
INSERT INTO training_record (user_id, train_date, duration, total_sets, total_reps, difficulty, is_completed, notes, create_time, update_time) VALUES
(1, '2026-05-18', 45, 0, 0, 1, 0, '今天刚开始练手臂，先做弯举热身', NOW(), NOW());
SET @record4_id = LAST_INSERT_ID();
INSERT INTO training_record_detail (record_id, exercise_id, set_number, reps, weight, is_completed, sort_order, create_time, update_time) VALUES
(@record4_id, 15,  1, 15,  20.0, 0, 1, NOW(), NOW()),
(@record4_id, 15,  2, 12,  22.5, 0, 2, NOW(), NOW()),
(@record4_id, 15,  3, 10,  25.0, 0, 3, NOW(), NOW()),
(@record4_id, 16,  1, 12,  12.5, 0, 4, NOW(), NOW()),
(@record4_id, 16,  2, 10,  15.0, 0, 5, NOW(), NOW()),
(@record4_id, 17,  1, 15,  15.0, 0, 6, NOW(), NOW()),
(@record4_id, 17,  2, 12,  17.5, 0, 7, NOW(), NOW()),
(@record4_id, 17,  3, 12,  17.5, 0, 8, NOW(), NOW());

-- ============================================================
-- 六、管理员训练计划（用户ID=1 admin）
-- ============================================================

-- 计划1：胸部增肌计划
INSERT INTO plan (user_id, group_id, title, description, target_body_part, difficulty_level, train_days, is_public, create_time, update_time) VALUES
(1, 2, '胸部增肌强化计划', '针对胸大肌的增肌训练，包含上中下胸全方位刺激，每周2次', '胸部', 2, 3, 1, NOW(), NOW());
SET @plan1_id = LAST_INSERT_ID();
INSERT INTO plan_detail (plan_id, exercise_id, day_number, sets, reps, weight, rest_seconds, sort_order, create_time, update_time) VALUES
(@plan1_id, 1,  1, 4, 10, 60.0, 90, 1, NOW(), NOW()),
(@plan1_id, 2,  1, 3, 12, 25.0, 90, 2, NOW(), NOW()),
(@plan1_id, 3,  1, 3, 15, 12.5, 60, 3, NOW(), NOW()),
(@plan1_id, 4,  1, 3, 12, 20.0, 60, 4, NOW(), NOW()),
(@plan1_id, 1,  2, 4,  8, 65.0, 90, 5, NOW(), NOW()),
(@plan1_id, 2,  2, 3, 10, 27.5, 90, 6, NOW(), NOW()),
(@plan1_id, 3,  2, 4, 12, 15.0, 60, 7, NOW(), NOW()),
(@plan1_id, 1,  3, 4,  8, 70.0, 90, 8, NOW(), NOW()),
(@plan1_id, 4,  3, 3, 12, 22.5, 60, 9, NOW(), NOW());

-- 计划2：背部增肌计划
INSERT INTO plan (user_id, group_id, title, description, target_body_part, difficulty_level, train_days, is_public, create_time, update_time) VALUES
(1, 2, '背部宽度与厚度训练', '引体向上+划船组合，打造V型背部', '背部', 3, 3, 1, NOW(), NOW());
SET @plan2_id = LAST_INSERT_ID();
INSERT INTO plan_detail (plan_id, exercise_id, day_number, sets, reps, weight, rest_seconds, sort_order, create_time, update_time) VALUES
(@plan2_id, 6,  1, 4,  8,  0.0, 120, 1, NOW(), NOW()),
(@plan2_id, 7,  1, 4, 10, 60.0, 120, 2, NOW(), NOW()),
(@plan2_id, 8,  1, 4, 12, 55.0,  90, 3, NOW(), NOW()),
(@plan2_id, 9,  1, 3, 12, 45.0,  90, 4, NOW(), NOW()),
(@plan2_id, 6,  2, 4, 10,  0.0, 120, 5, NOW(), NOW()),
(@plan2_id, 7,  2, 4,  8, 65.0, 120, 6, NOW(), NOW()),
(@plan2_id, 10, 2, 4, 12, 25.0,  90, 7, NOW(), NOW()),
(@plan2_id, 8,  3, 4, 10, 60.0,  90, 8, NOW(), NOW()),
(@plan2_id, 9,  3, 4, 12, 50.0,  90, 9, NOW(), NOW());

-- 计划3：新手入门全身计划
INSERT INTO plan (user_id, group_id, title, description, target_body_part, difficulty_level, train_days, is_public, create_time, update_time) VALUES
(1, 1, '新手全身入门计划', '适合初学者的全身训练，每周3次，每动作3组', '全身', 1, 3, 1, NOW(), NOW());
SET @plan3_id = LAST_INSERT_ID();
INSERT INTO plan_detail (plan_id, exercise_id, day_number, sets, reps, weight, rest_seconds, sort_order, create_time, update_time) VALUES
(@plan3_id, 5,  1, 3, 15,  0.0, 60, 1, NOW(), NOW()),
(@plan3_id, 23, 1, 3, 12, 40.0, 90, 2, NOW(), NOW()),
(@plan3_id, 12, 1, 3, 12,  7.5, 60, 3, NOW(), NOW()),
(@plan3_id, 8,  1, 3, 12, 35.0, 60, 4, NOW(), NOW()),
(@plan3_id, 18, 2, 3, 45,  0.0, 90, 5, NOW(), NOW()),
(@plan3_id, 9,  2, 3, 12, 15.0, 60, 6, NOW(), NOW()),
(@plan3_id, 15, 2, 3, 12, 15.0, 60, 7, NOW(), NOW()),
(@plan3_id, 20, 3, 3, 12, 50.0, 90, 8, NOW(), NOW()),
(@plan3_id, 14, 3, 3, 12,  5.0, 60, 9, NOW(), NOW()),
(@plan3_id, 17, 3, 3, 15, 12.5, 60, 10, NOW(), NOW());

-- 计划4：肩部立体训练
INSERT INTO plan (user_id, group_id, title, description, target_body_part, difficulty_level, train_days, is_public, create_time, update_time) VALUES
(1, 2, '肩部立体塑形计划', '前束、中束、后束均衡训练，打造立体肩型', '肩部', 2, 2, 1, NOW(), NOW());
SET @plan4_id = LAST_INSERT_ID();
INSERT INTO plan_detail (plan_id, exercise_id, day_number, sets, reps, weight, rest_seconds, sort_order, create_time, update_time) VALUES
(@plan4_id, 11, 1, 4, 10, 22.5, 90, 1, NOW(), NOW()),
(@plan4_id, 12, 1, 4, 15, 10.0, 60, 2, NOW(), NOW()),
(@plan4_id, 13, 1, 3, 12, 12.5, 60, 3, NOW(), NOW()),
(@plan4_id, 14, 1, 4, 15, 10.0, 60, 4, NOW(), NOW()),
(@plan4_id, 11, 2, 4,  8, 25.0, 90, 5, NOW(), NOW()),
(@plan4_id, 12, 2, 3, 12, 12.5, 60, 6, NOW(), NOW()),
(@plan4_id, 14, 2, 4, 12, 12.5, 60, 7, NOW(), NOW());

-- 计划5：手臂增肌计划
INSERT INTO plan (user_id, group_id, title, description, target_body_part, difficulty_level, train_days, is_public, create_time, update_time) VALUES
(1, 2, '手臂围度突破计划', '二头肌与三头肌分化训练，提升手臂线条', '手臂', 2, 2, 1, NOW(), NOW());
SET @plan5_id = LAST_INSERT_ID();
INSERT INTO plan_detail (plan_id, exercise_id, day_number, sets, reps, weight, rest_seconds, sort_order, create_time, update_time) VALUES
(@plan5_id, 15, 1, 4, 10, 30.0, 90, 1, NOW(), NOW()),
(@plan5_id, 16, 1, 3, 12, 15.0, 60, 2, NOW(), NOW()),
(@plan5_id, 17, 1, 4, 12, 25.0, 60, 3, NOW(), NOW()),
(@plan5_id, 18, 1, 3, 10, 40.0, 90, 4, NOW(), NOW()),
(@plan5_id, 15, 2, 3, 12, 27.5, 90, 5, NOW(), NOW()),
(@plan5_id, 16, 2, 4, 12, 17.5, 60, 6, NOW(), NOW()),
(@plan5_id, 17, 2, 4, 15, 22.5, 60, 7, NOW(), NOW());

-- 计划6：腿部力量进阶
INSERT INTO plan (user_id, group_id, title, description, target_body_part, difficulty_level, train_days, is_public, create_time, update_time) VALUES
(1, 4, '腿部力量进阶计划', '深蹲硬拉腿举组合，系统提升下肢力量', '腿部', 3, 2, 1, NOW(), NOW());
SET @plan6_id = LAST_INSERT_ID();
INSERT INTO plan_detail (plan_id, exercise_id, day_number, sets, reps, weight, rest_seconds, sort_order, create_time, update_time) VALUES
(@plan6_id, 19, 1, 5,  5, 100.0, 120, 1, NOW(), NOW()),
(@plan6_id, 20, 1, 4,  8,  80.0, 120, 2, NOW(), NOW()),
(@plan6_id, 21, 1, 4, 10, 150.0, 90, 3, NOW(), NOW()),
(@plan6_id, 22, 1, 3, 12, 200.0, 90, 4, NOW(), NOW()),
(@plan6_id, 19, 2, 4,  8,  90.0, 120, 5, NOW(), NOW()),
(@plan6_id, 20, 2, 3, 10,  85.0, 120, 6, NOW(), NOW()),
(@plan6_id, 21, 2, 4,  8, 160.0, 90, 7, NOW(), NOW());

-- 计划7：核心稳定强化
INSERT INTO plan (user_id, group_id, title, description, target_body_part, difficulty_level, train_days, is_public, create_time, update_time) VALUES
(1, 1, '核心稳定强化计划', '平板支撑与卷腹组合，增强腰腹核心力量', '核心', 1, 3, 1, NOW(), NOW());
SET @plan7_id = LAST_INSERT_ID();
INSERT INTO plan_detail (plan_id, exercise_id, day_number, sets, reps, weight, rest_seconds, sort_order, create_time, update_time) VALUES
(@plan7_id, 23, 1, 3, 60,  5.0, 45, 1, NOW(), NOW()),
(@plan7_id, 25, 1, 4, 20,  2.5, 45, 2, NOW(), NOW()),
(@plan7_id, 26, 1, 3, 20,  5.0, 45, 3, NOW(), NOW()),
(@plan7_id, 24, 2, 3, 15,  7.5, 45, 4, NOW(), NOW()),
(@plan7_id, 25, 2, 4, 15,  5.0, 45, 5, NOW(), NOW()),
(@plan7_id, 26, 2, 3, 25,  7.5, 45, 6, NOW(), NOW()),
(@plan7_id, 23, 3, 3, 45,  5.0, 45, 7, NOW(), NOW()),
(@plan7_id, 24, 3, 3, 20, 10.0, 45, 8, NOW(), NOW());

-- 计划8：胸背超级组
INSERT INTO plan (user_id, group_id, title, description, target_body_part, difficulty_level, train_days, is_public, create_time, update_time) VALUES
(1, 2, '胸背超级组计划', '推拉超级组训练，高效刺激上半身', '胸部/背部', 3, 2, 1, NOW(), NOW());
SET @plan8_id = LAST_INSERT_ID();
INSERT INTO plan_detail (plan_id, exercise_id, day_number, sets, reps, weight, rest_seconds, sort_order, create_time, update_time) VALUES
(@plan8_id,  1, 1, 4, 10, 55.0, 75, 1, NOW(), NOW()),
(@plan8_id,  7, 1, 4, 10, 50.0, 75, 2, NOW(), NOW()),
(@plan8_id,  2, 1, 3, 12, 22.5, 75, 3, NOW(), NOW()),
(@plan8_id,  8, 1, 4, 12, 45.0, 75, 4, NOW(), NOW()),
(@plan8_id,  1, 2, 4,  8, 60.0, 90, 5, NOW(), NOW()),
(@plan8_id,  9, 2, 4, 12, 40.0, 75, 6, NOW(), NOW()),
(@plan8_id,  4, 2, 3, 15, 17.5, 60, 7, NOW(), NOW()),
(@plan8_id, 10, 2, 3, 12, 20.0, 75, 8, NOW(), NOW());

-- 计划9：减脂力量循环
INSERT INTO plan (user_id, group_id, title, description, target_body_part, difficulty_level, train_days, is_public, create_time, update_time) VALUES
(1, 3, '减脂力量循环计划', '全身复合动作为主，高次数短休息，提升代谢', '全身', 2, 4, 1, NOW(), NOW());
SET @plan9_id = LAST_INSERT_ID();
INSERT INTO plan_detail (plan_id, exercise_id, day_number, sets, reps, weight, rest_seconds, sort_order, create_time, update_time) VALUES
(@plan9_id,  5, 1, 4, 20,  2.5, 45, 1, NOW(), NOW()),
(@plan9_id, 19, 1, 4, 15, 60.0, 60, 2, NOW(), NOW()),
(@plan9_id,  8, 1, 4, 15, 35.0, 45, 3, NOW(), NOW()),
(@plan9_id, 11, 1, 3, 15, 15.0, 45, 4, NOW(), NOW()),
(@plan9_id,  6, 2, 4, 12,  5.0, 45, 5, NOW(), NOW()),
(@plan9_id, 21, 2, 4, 12, 80.0, 60, 6, NOW(), NOW()),
(@plan9_id,  3, 2, 3, 15, 12.5, 45, 7, NOW(), NOW()),
(@plan9_id, 25, 2, 3, 20,  5.0, 45, 8, NOW(), NOW()),
(@plan9_id,  7, 3, 4, 12, 45.0, 60, 9, NOW(), NOW()),
(@plan9_id, 22, 3, 4, 15, 120.0, 60, 10, NOW(), NOW());

-- 计划10：四分化增肌
INSERT INTO plan (user_id, group_id, title, description, target_body_part, difficulty_level, train_days, is_public, create_time, update_time) VALUES
(1, 2, '四分化增肌计划', '胸、背、腿、肩四日分化，系统堆叠肌肉量', '全身', 3, 4, 1, NOW(), NOW());
SET @plan10_id = LAST_INSERT_ID();
INSERT INTO plan_detail (plan_id, exercise_id, day_number, sets, reps, weight, rest_seconds, sort_order, create_time, update_time) VALUES
(@plan10_id,  1, 1, 4, 10, 62.5, 90, 1, NOW(), NOW()),
(@plan10_id,  2, 1, 3, 12, 25.0, 90, 2, NOW(), NOW()),
(@plan10_id,  3, 1, 3, 15, 15.0, 60, 3, NOW(), NOW()),
(@plan10_id,  7, 2, 4, 10, 62.5, 90, 4, NOW(), NOW()),
(@plan10_id,  8, 2, 4, 12, 52.5, 90, 5, NOW(), NOW()),
(@plan10_id,  9, 2, 3, 12, 42.5, 75, 6, NOW(), NOW()),
(@plan10_id, 19, 3, 5,  5, 105.0, 120, 7, NOW(), NOW()),
(@plan10_id, 20, 3, 4,  8,  82.5, 120, 8, NOW(), NOW()),
(@plan10_id, 21, 3, 3, 10, 155.0, 90, 9, NOW(), NOW()),
(@plan10_id, 11, 4, 4, 10, 25.0, 90, 10, NOW(), NOW()),
(@plan10_id, 12, 4, 4, 15, 12.5, 60, 11, NOW(), NOW()),
(@plan10_id, 14, 4, 4, 15, 12.5, 60, 12, NOW(), NOW());

-- 计划11：推日专项
INSERT INTO plan (user_id, group_id, title, description, target_body_part, difficulty_level, train_days, is_public, create_time, update_time) VALUES
(1, 4, '推日力量专项计划', '卧推推举窄距卧推，专注推举类动作力量提升', '胸部/肩部/手臂', 3, 2, 1, NOW(), NOW());
SET @plan11_id = LAST_INSERT_ID();
INSERT INTO plan_detail (plan_id, exercise_id, day_number, sets, reps, weight, rest_seconds, sort_order, create_time, update_time) VALUES
(@plan11_id,  1, 1, 5,  5, 75.0, 120, 1, NOW(), NOW()),
(@plan11_id,  2, 1, 4,  8, 27.5, 90, 2, NOW(), NOW()),
(@plan11_id, 11, 1, 4, 10, 25.0, 90, 3, NOW(), NOW()),
(@plan11_id, 18, 1, 4,  8, 45.0, 90, 4, NOW(), NOW()),
(@plan11_id,  1, 2, 4,  8, 70.0, 120, 5, NOW(), NOW()),
(@plan11_id,  4, 2, 3, 12, 20.0, 60, 6, NOW(), NOW()),
(@plan11_id, 17, 2, 4, 12, 27.5, 60, 7, NOW(), NOW());

-- 计划12：拉日专项
INSERT INTO plan (user_id, group_id, title, description, target_body_part, difficulty_level, train_days, is_public, create_time, update_time) VALUES
(1, 4, '拉日力量专项计划', '划船下拉硬拉组合，强化背部与后链', '背部/腿部', 3, 2, 1, NOW(), NOW());
SET @plan12_id = LAST_INSERT_ID();
INSERT INTO plan_detail (plan_id, exercise_id, day_number, sets, reps, weight, rest_seconds, sort_order, create_time, update_time) VALUES
(@plan12_id,  7, 1, 5,  5, 70.0, 120, 1, NOW(), NOW()),
(@plan12_id,  8, 1, 4, 10, 57.5, 90, 2, NOW(), NOW()),
(@plan12_id,  9, 1, 4, 12, 47.5, 90, 3, NOW(), NOW()),
(@plan12_id,  6, 1, 4,  8,  5.0, 90, 4, NOW(), NOW()),
(@plan12_id, 20, 2, 4,  6, 90.0, 120, 5, NOW(), NOW()),
(@plan12_id, 10, 2, 3, 12, 22.5, 75, 6, NOW(), NOW()),
(@plan12_id, 21, 2, 4,  8, 165.0, 90, 7, NOW(), NOW());

-- 计划13：周末全身激活
INSERT INTO plan (user_id, group_id, title, description, target_body_part, difficulty_level, train_days, is_public, create_time, update_time) VALUES
(1, 1, '周末全身激活计划', '轻中等强度全身训练，适合周末恢复性训练', '全身', 1, 2, 1, NOW(), NOW());
SET @plan13_id = LAST_INSERT_ID();
INSERT INTO plan_detail (plan_id, exercise_id, day_number, sets, reps, weight, rest_seconds, sort_order, create_time, update_time) VALUES
(@plan13_id,  5, 1, 3, 15,  2.5, 60, 1, NOW(), NOW()),
(@plan13_id, 12, 1, 3, 15,  7.5, 60, 2, NOW(), NOW()),
(@plan13_id,  8, 1, 3, 12, 30.0, 60, 3, NOW(), NOW()),
(@plan13_id, 16, 1, 3, 12, 12.5, 60, 4, NOW(), NOW()),
(@plan13_id, 22, 2, 3, 12, 100.0, 75, 5, NOW(), NOW()),
(@plan13_id, 25, 2, 3, 15,  5.0, 45, 6, NOW(), NOW()),
(@plan13_id, 17, 2, 3, 12, 20.0, 60, 7, NOW(), NOW());
