...
> 提示：`-p123456` 是密码紧跟在 -p 后面（无空格）。如果使用 `-p` 会交互式提示输入密码。
> **重要**：必须加上 `--default-character-set=utf8mb4`，否则导入中文数据时报错 `Data too long for column`。
...
### 第四步：部署到 Tomcat 9.0.118

**1. 下载 Tomcat**

从 [Apache Tomcat 官网](https://tomcat.apache.org/download-90.cgi) 下载 Tomcat 9.0.118 的 zip 包，解压到本地目录（例如 `D:/Develop\apache-tomcat-9.0.118`）。
